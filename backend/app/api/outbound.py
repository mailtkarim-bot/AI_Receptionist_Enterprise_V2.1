...
async def create_campaign(data: CampaignCreate, business: Business = Depends(get_current_business), db: AsyncSession = Depends(get_db)):
    check_feature_access(business, "campaigns")
    campaign = Campaign(business_id=business.id, name=data.name, type=data.type, target_audience=data.target_audience, message_template=data.message_template, schedule=data.schedule)
    db.add(campaign)
    await db.commit()
